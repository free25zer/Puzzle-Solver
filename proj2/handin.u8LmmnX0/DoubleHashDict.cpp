...
int base = hash1(keyID);
int offset = hash2(keyID); 
//cout << "Find Base Hash   " << base << endl; 
//cout << "Find Offset Hash   " << offset <<endl;
for(int i = base; table[i].key != NULL; i = ((i + offset) % size)){
    if(table[i].key->getUniqId() == keyID){
...
void DoubleHashDict::add(MazeState *key, MazeState *pred) {
  ...
  // cout << i << endl;
  int offset = hash2(keyID);
  for(i; table[i].key != NULL; i = ((i + offset) % size)){
    if(table[i].key == key){
      table[i].data = pred;
    }
  }
